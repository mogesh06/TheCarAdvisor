...
while ($loan = mysqli_fetch_assoc($result)) {
    ...
    $dueResult = mysqli_query($con, "SELECT * FROM $vno WHERE DueDate <= '$dateToday' AND RecAmt IS NULL ORDER BY DueDate ASC LIMIT 1");
  ...
  <tr style ="border: 1px solid black; border-collapse: collapse; color:red"">
    ...
    <tr style ="border: 1px solid black; border-collapse: collapse"; color:red">
    <th style="font-size: 19px; border: 1px solid black; border-collapse: collapse; ; padding: 8px;">Due Amount:</th>
    <td style ="font-size: 17px; border: 1px solid black; border-collapse: collapse; ; padding: 8px;"><i>' . $due['DueAmt'] . '</i></td>
  </tr>
  </table>
